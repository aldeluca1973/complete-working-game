...
export function StrategicCourtroom({ onGameEnd }: StrategicCourtroomProps) {
  ...
  useEffect(() => {
    ...
    const loadGameData = async () => {
      try {
        console.log('Loading game data for session:', currentGame.id)
        
        // Load game session data
        const { data: gameSessionData, error: gameError } = await supabase
          .from('game_sessions')
          .select('*')
        ...
        // Set up trial events
        generateTrialEvents(caseData)
      } catch (error) {
        console.error('Error loading game data:', error)
        addNotification('Failed to load game data. Please try again.')
      }
    }
    ...
    const loadFallbackData = async () => {
      try {
        ...
        setCaseData(testCase)
        
        // Generate mock evidence
        const mockEvidence: EvidenceItem[] = testCase.evidence || []
        setEvidenceItems(mockEvidence)
        
        // Set up trial events
        generateTrialEvents(testCase)
      } catch (error) {
